 ...
 **/
public class SimpleDescriptionWordCount extends Configured implements Tool {
	private static final Logger LOG = Logger.getLogger(SimpleDescriptionWordCount.class);
	 ...
	 * @version 1.0, December 1, 2014
	 **/
	public static class SimpleDescriptionWordCountMapper extends Mapper<Text, Text, Text, IntWritable> {
		 ...
		 * <p>
		 * 
		 * No additional error checking is performed against the filtered NEISS data as all cleansing of the data should
		 * have been performed as the raw NEISS data was processed.
		 * <p>
		 * 
		...
		protected void map(Text aKey, Text aValue, Context aContext) throws IOException, InterruptedException {
			...
			for (String descriptionWord : WORD_BOUNDARY.split(descriptionLine)) {
				 ...
				 * 		[KEY(Text(Description_Word))  VALUE(IntWritable(1))]
				 */
				// @formatter:on
				xWordKey.set(descriptionWord);
		 ...
		 * counts for the incident Description text.
		 * 
		 * @param aContext the context object associated with this process
		 **/
		@Override
	 ...
	 * @version 1.0, December 1, 2014
	 **/
	public static class SimpleSumReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
		...
		protected void reduce(Text aKey, Iterable<IntWritable> aValues, Context aContext) throws IOException,
			...
			/*
			 * Loop through all of the values collected for each key (DescriptionWord) from the Mapper<> and summarize
			 * the total number of times that the word was encountered within the data.
			 */
			int myWordCount = 0;
			...
			/*
			 * Write the output record in the following format: 
			 * 		[KEY(Text(Description_Word))  VALUE(IntWritable(SUM(Description_Word_Count)))]
			 */
			// @formatter:on
	...
	public int run(String[] aArguments) throws Exception {
		...
		job.setJarByClass(SimpleDescriptionWordCount.class);
		job.setJobName("WordCount against the NEISS Descriptions");

		job.setMapperClass(SimpleDescriptionWordCountMapper.class);
		job.setReducerClass(SimpleSumReducer.class);
		job.setCombinerClass(SimpleSumReducer.class);
	...
	public static void main(String[] aArguments) throws Exception {
		int exitCode = ToolRunner.run(new Configuration(), new SimpleDescriptionWordCount(), aArguments);
